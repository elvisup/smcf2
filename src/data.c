				...
				if (module_chn_info.chn_info[k].producer_id == rs_mid) {
					if (r_mchn_attr == NORMAL_CHN) {
						if (module_chn_info.chn_info[k].producer_chn_id == rs_mchnid) {
							...
							/*
							 *slog(LOG_DBG, "%s:%d normal sender & recv chn match!, nodelist: 0x%x\n", \
							 *                __func__, __LINE__, module_chn_info.chn_info[k].chn_node_list);
							 *slog(LOG_DBG, "%s:%d Sender: mid: %d. cid: %d\n", __func__, __LINE__, rs_mid, rs_mchnid);
							 *slog(LOG_DBG, "%s:%d Recevr: mid: %d. cid: %d\n", __func__, __LINE__, r_mid, r_mchnid);
							 */
							break;
						}
						...
						if (module_chn_info.chn_info[k].producer_hchn_id == rs_mchnid) {
							...
							/*
							 *slog(LOG_DBG, "%s:%d hook sender & recv chn match!, nodelist: 0x%x\n", \
							 *                __func__, __LINE__, module_chn_info.chn_info[k].chn_node_list);
							 *slog(LOG_DBG, "%s:%d Sender: mid: %d. cid: %d\n", __func__, __LINE__, rs_mid, rs_mchnid);
							 *slog(LOG_DBG, "%s:%d Recevr: mid: %d. cid: %d\n", __func__, __LINE__, r_mid, r_mchnid);
							 */
...
int read_data(int current_module_id, int channel_id, void **data, int chn_type)
{
	module_t *cur_module = NULL;
...
int write_data(int context, int current_module_id, int channel_id, void **data, int chn_type)
{
	...
	if (module_role == DATA_CHANNEL_SENDER) {
		...
		if (chn_type == HOOK_CHN) {
			...
			if (sender_module->data_hchannel[cid].release)
				sender_module->data_hchannel[cid].release(node->data);
			else {
